...
@media (prefers-reduced-motion: reduce){
  ...
  var GardenLayoutService = {
    ...
    async getGardenForPlayerId(playerId) {
  ...
  function resolvePlayerLabel(slot, fallbackId, slotKey, nameMap) {
    const mapped = nameMap?.get(fallbackId);
    if (mapped && mapped.trim()) return mapped.trim();
    const direct = slot?.name ?? slot?.playerName ?? slot?.displayName ?? slot?.username ?? slot?.data?.playerName ?? slot?.data?.displayName ?? slot?.data?.name ?? slot?.data?.username ?? slot?.data?.player?.name ?? slot?.data?.player?.displayName ?? slot?.data?.player?.username ?? slot?.data?.player?.user?.name ?? slot?.data?.player?.user?.displayName ?? slot?.data?.player?.user?.username ?? slot?.data?.user?.name ?? slot?.data?.user?.displayName ?? slot?.data?.user?.username;
    const label = typeof direct === "string" && direct.trim() ? direct.trim() : "";
    if (label) return label;
    if (slotKey && slotKey.trim()) return `Player ${slotKey}`;
    return fallbackId;
  }
  function extractPlayerNameMap(state2) {
    const map = /* @__PURE__ */ new Map();
    const add = (entry) => {
      ...
      if (!id) return;
      const nameRaw = entry.name ?? entry.displayName ?? entry.username ?? entry.user?.name ?? entry.user?.displayName ?? entry.user?.username;
      const name = typeof nameRaw === "string" ? nameRaw.trim() : "";
      if (name) map.set(id, name);
    };
    const candidates = [
      state2?.data?.players,
      state2?.child?.data?.players,
      state2?.child?.data?.playersById,
      state2?.child?.data?.room?.players
    ];
  ...
  async function listLobbyPlayers() {
    try {
      const cur = await Store.select("stateAtom");
      const slots = cur?.child?.data?.userSlots;
      ...
      const pushSlot = (slot, slotKey, idx) => {
        if (!slot || typeof slot !== "object") return;
        const idRaw = slot?.playerId ?? slot?.id ?? slot?.data?.playerId ?? slot?.data?.id ?? "";
        const id = String(idRaw || "").trim();
        if (!id) return;
        if (selfId && id === selfId) return;
        const slotIndex = Number.isFinite(idx) ? idx : slotKey && Number.isFinite(Number(slotKey)) ? Number(slotKey) : null;
  ...
  function renderEditorMenu(container) {
    ...
    let applyExternalGarden = null;
    const renderLayoutTab = (view) => {
      view.innerHTML = "";
    ...
    const renderDevTab = (view) => {
      ...
      const hydratePlayers = async () => {
        ...
        for (const p of list) {
          const opt = document.createElement("option");
          opt.value = p.id;
          const slotLabel = Number.isFinite(p.slotIndex) ? ` (#${p.slotIndex})` : "";
