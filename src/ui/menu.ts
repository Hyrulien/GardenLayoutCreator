// src/ui/menu.ts
// Generic tabbed menu, compact, with UI helpers + LS persistence.
// + Reusable helpers: split2 (2-column layout) and VTabs (vertical tabs)
...
export type VTabItem = {
  ...
  subtitle?: string;
  avatarUrl?: string;
  statusColor?: string;   // e.g. "#48d170" (left dot)
  badge?: string | null;  // small counter/flag on the right of the label
  disabled?: boolean;
};
...
export type VTabsOptions = {
  ...
  emptyText?: string;
  /** initial selection (id) */
  initialId?: string | null;
  /** selection callback */
  onSelect?: (id: string | null, item: VTabItem | null) => void;
  /** max list height (overflow:auto). If absent => no limit */
  maxHeightPx?: number;
...
export class Menu {
  ...
  public applyTranslations = (target: ParentNode | null) => {
    if (!target) return;
    try { i18n.applyTo?.(target); } catch {}
  };
  private translateTitle = (t: string) => {
    try { return i18n.translateString?.(t) ?? t; } catch { return t; }
  };
  ...
  mount(container: HTMLElement) {
    ...
    // create views already registered
    if (this.tabs.size) {
      for (const [id, def] of this.tabs) this.createTabView(id, def);
      this.restoreActive();
    }

    // if no tab => hide the bar (avoids an empty row)
    ...
    // Apply current language to freshly mounted menu
    this.applyTranslations(this.root);
  }
  ...
  refreshTab(id: string) {
    ...
    // --- Preserve scroll of the nearest scrollable container ---
  ...
  private findScrollableAncestor(start: HTMLElement): HTMLElement | null {
    ...
    // fallback: HUD window
    return document.querySelector('.glc-win') as HTMLElement | null;
  }
  ...
  private _onKey = (e: KeyboardEvent) => {
    ...
    const alt = e.altKey || this._insertDown;   // current state of ALT or Insert
    if (alt !== this._altDown) {
      this._altDown = alt;
      this._updateAltCursor();
    }
  };
  private _onBlur = () => {                // reset when leaving the window/tab
    this._altDown = false;
  ...
  private _updateAltCursor() {
    ...
    // ⇧ if you want this ONLY on the tab bar,
...
inputNumber(min = 0, max = 9999, step = 1, value = 0) {
  ...
  // vertical stack on the right (separate from the field)
  ...
  const moveIndicator = (animate = false) => {
    ...
    // left/width relative to the container *padding box*
    ...
    if (n === 1) {
      ...
      // first -> include half gap on the right + left padding
      const rightEdge = left + width + gap / 2;
      left = 0;
      width = rightEdge - left;
    } else if (i === n - 1) {
      // last -> half gap on the left + right padding
      left  = left - gap / 2;
      width = padW - left;
    } else {
      // middle -> half gap on each side
      left  = left  - gap / 2;
      width = width + gap;
    }
  ...
  // first position after insertion in the DOM
 ...
 hotkeyButton(
  ...
  const stopRecording = (commit: boolean) => {
    ...
    // commit already done in handleKeyDown if valid
    render();
  };
  ...
  btn.addEventListener("click", (e) => {
    ...
    if (!recording) {
      ...
      // small trick: visual focus only in recording mode (already handled by .is-recording)
      btn.focus();
    }
  });
  ...
  private createTabView(id: string, def: TabDef) {
    ...
    // enable by default if no selection
  ...
  private updateTabsBarVisibility() {
    ...
    if (hasTabs) {
        // ensure the tab bar is present BEFORE .qmm-views
        ...
        // remove it from the DOM completely to avoid leftover space
        if (this.tabBar.parentElement) {
